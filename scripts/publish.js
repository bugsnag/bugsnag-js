...
function publish (publishUrl) {
  ...
  if (myVersions.length === 0) {
    ...
    common.run(`./node_modules/.bin/lerna publish ${version} --dist-tag ${distTag} --exact --yes --force-publish --no-push --no-git-tag-version --no-verify-access --registry ${publishUrl}`, true)
