...
module.exports = {
  configureIOSProject: function configureIOSProject (fixtureDir, reactNativeVersion) {
    ...
    // Strip `quirks_mode` for all RN versions: newer CLI templates emit it but
    // `use_react_native!` on RN <= 0.74 rejects it as an unknown keyword.
    if (/quirks_mode/.test(podfileContents)) {
      podfileContents = podfileContents.replace(/^\s*(?::)?quirks_mode\s*(?:=>|:)\s*.*,\s*$\n?/gm, '')
      fs.writeFileSync(`${fixtureDir}/ios/Podfile`, podfileContents)
    }
