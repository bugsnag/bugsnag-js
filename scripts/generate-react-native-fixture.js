...
let reactNavigationNativeSafeContextVersion = '^5.0.0'
...
switch (true) {
  ...
  case rnVersion >= 0.73:
    ...
    reactNavigationNativeSafeContextVersion = '4.14.0'
  ...
  case rnVersion <= 0.69:
    ...
    reactNavigationNativeSafeContextVersion = '4.3.4'
...
const REACT_NAVIGATION_PEER_DEPENDENCIES = [
  ...
  `react-native-safe-area-context@${reactNavigationNativeSafeContextVersion}`
...
if (!process.env.SKIP_GENERATE_FIXTURE) {
  ...
  // Configure Android cleartext traffic and storage permissions for BitBar / Android 14+

  configureAndroidFixtureForBitBar(fixtureDir)
...
if (process.env.BUILD_ANDROID === 'true' || process.env.BUILD_ANDROID === '1') {
  ...
  // Ensure Android manifest configuration is present before building APK
...
* Injects network security config, cleartext HTTP support, and storage permissions

* to ensure Maze Runner communication and fixture.config reading work on physical

* Android 14/15 devices running in BitBar without local tunneling.

*/

function configureAndroidFixtureForBitBar (fixtureDirectory) {
...
`
  ...
  const missingPermissions = requiredPermissions
    ...
    .filter(perm => !manifest.includes(perm.split(' ')[1]))

    .join('\n    ')
 
  if (missingPermissions) {

    manifest = manifest.replace('<application', `${missingPermissions}\n    <application`)

  }
 
  // Add cleartext traffic and network security config to <application>
  ...
  console.log('[Android] Configured cleartext traffic, storage permissions & network security config for BitBar')
