...
Scenario: multiple notify() calls with different grouping discriminators
  ...
  And the error payload field "events.0.exceptions.0.message" equals "null-discriminator"
  And the error payload field "events.0.groupingDiscriminator" is null
  And I discard the oldest error

  # Fifth error - cleared discriminator on event (undefined)
  And the error payload field "events.0.exceptions.0.message" equals "undefined-discriminator"
  And the error payload field "events.0.groupingDiscriminator" is null
  And I discard the oldest error

  # Sixth error - cleared discriminator on client and event
  And the error payload field "events.0.exceptions.0.message" equals "no-discriminator-2"
  And the error payload field "events.0.groupingDiscriminator" is null
