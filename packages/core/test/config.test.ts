...
describe('config', () => {
  describe('schema', () => {
    it('has the required properties { validate(), defaultValue(), message }', () => {
      Object.keys(schema).forEach(k => {
        ...
        definition.validate('stringy stringerson')
        definition.validate(['foo', 'bar', 'baz'])
        definition.validate(new Date())
  ...
  describe('enabledBreadcrumbTypes', () => {
    it('fails when a supplied value is not a valid breadcrumb type', () => {
      const enabledBreadcrumbTypesValidator =
        schema.enabledBreadcrumbTypes.validate

      expect(
        enabledBreadcrumbTypesValidator(['UNKNOWN_BREADCRUMB_TYPE'])
      ).toBe(false)
    })
  })
  ...
  describe('enabledErrorTypes', () => {
    ...
    it('works with a subset of error types', () => {
      ...
      expect(
        enabledErrorTypesValidator({ unhandledExceptions: true })
      ).toBe(true)
    })
  ...
  describe('featureFlags', () => {
    ...
    it('fails when a value does not have a "name"', () => {
      ...
      expect(
        validator([{ name: 'hello' }, { notName: 'oops' }])
      ).toBe(false)
    })
