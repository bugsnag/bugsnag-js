...
describe('plugin: strip project root', () => {
  it('should remove the project root if it matches the start of the stackframe’s file', done => {
    ...
    client.delivery(client => ({
      sendReport: (report) => {
        ...
        expect(evt.get('stacktrace')[0].file).toBe(join('lib', '01.js'))
        expect(evt.get('stacktrace')[1].file).toBe(join('lib', '02.js'))
        expect(evt.get('stacktrace')[2].file).toBe(join('lib', '03.js'))
        done()
      },
      sendSession: () => {}
    }))
  ...
  it('should not remove a matching substring if it is not at the start', done => {
    ...
    client.delivery(client => ({
      sendReport: (report) => {
        const evt = report.events[0]
        expect(evt.get('stacktrace')[0].file).toBe(join('/var', 'lib', '01.js'))
        expect(evt.get('stacktrace')[1].file).toBe(join('/foo', 'lib', '02.js'))
        expect(evt.get('stacktrace')[2].file).toBe(join('/tmp', 'lib', '03.js'))
        done()
      },
      sendSession: () => {}
    }))
  ...
  it('should work with node_modules and node internals', done => {
    ...
    client.delivery(client => ({
      sendReport: (report) => {
        const evt = report.events[0]
        expect(evt.get('stacktrace')[0].file).toBe('_module.js')
        expect(evt.get('stacktrace')[1].file).toBe(join('node_modules', 'bugsnag-example', 'index.js'))
        done()
      },
      sendSession: () => {}
    }))
  ...
  it('should tolerate stackframe.file not being a string', done => {
    ...
    client.delivery(client => ({
      sendReport: (report) => {
        const evt = report.events[0]
        expect(evt.get('stacktrace')[0].file).toBe('global code')
        expect(evt.get('stacktrace')[1].file).toBe('global code')
        expect(evt.get('stacktrace')[2].file).toEqual({})
        done()
      },
      sendSession: () => {}
    }))
