...
function wrapHandler (client, flushTimeoutMs, handler) {
  return async function (request, env, ctx) {
    // Clone the client for this request to avoid accumulating callbacks
    const requestClient = clone(client)

    // Only start session and add request metadata if no server plugin is loaded,
    ...
    // This allows Bugsnag.notify() calls within the handler to use the request-scoped client
